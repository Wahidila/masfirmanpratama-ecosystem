...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Metode Penarikan</h1>
</div>

<div class="max-w-2xl space-y-4">
    @if ($method->withdrawals()->exists())
        <x-alert tone="info">
            Metode ini sudah dipakai pada riwayat penarikan. Mengganti namanya tidak mengubah riwayat lama — nama metode dicatat terpisah saat pengajuan.
        </x-alert>
    @endif

    <x-card>
        <form method="POST" action="{{ route('admin.withdrawal-methods.update', $method) }}" class="space-y-4">
            @csrf @method('PUT')

            <div class="grid sm:grid-cols-2 gap-4">
                <x-form.group label="Nama Metode" name="name" required>
                    <x-form.input name="name" value="{{ old('name', $method->name) }}" required />
                </x-form.group>
                <x-form.group label="Tipe" name="type" required>
                    <x-form.select name="type" required>
                        @foreach (\App\Models\WithdrawalMethod::TYPES as $value => $label)
                            <option value="{{ $value }}" @selected(old('type', $method->type) === $value)>{{ $label }}</option>
                        @endforeach
                    </x-form.select>
                </x-form.group>
            </div>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <x-form.group label="Minimum Penarikan (Rp)" name="min_withdrawal" required
                    ...
                    <x-form.input type="number" name="min_withdrawal" value="{{ old('min_withdrawal', (int) $method->min_withdrawal) }}" min="1" step="1" required />
                </x-form.group>
                <x-form.group label="Biaya Admin (Rp)" name="fee_flat" required
                    ...
                    <x-form.input type="number" name="fee_flat" value="{{ old('fee_flat', (int) $method->fee_flat) }}" min="0" step="1" required />
                </x-form.group>
            </div>
            ...
            <x-form.checkbox name="is_active" value="1" :checked="(bool) old('is_active', $method->is_active)" label="Aktif — bisa dipilih affiliator" />

            <div class="flex items-center gap-2 pt-2">
                <x-button type="submit" icon="save">Simpan Perubahan</x-button>
