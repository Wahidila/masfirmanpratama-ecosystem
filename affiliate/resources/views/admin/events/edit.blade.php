...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold text-slate-800 mt-2">Edit Event: {{ $event->title }}</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.events.update', $event) }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $event->title) }}" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <select name="type" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" required>
                    <option value="challenge" {{ old('type', $event->type) === 'challenge' ? 'selected' : '' }}>Challenge</option>
                    <option value="contest" {{ old('type', $event->type) === 'contest' ? 'selected' : '' }}>Contest</option>
                    <option value="bonus" {{ old('type', $event->type) === 'bonus' ? 'selected' : '' }}>Bonus</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="3" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">{{ old('description', $event->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date', $event->start_date->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date', $event->end_date->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" required>
            ...
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" required>
                    ...
                    <option value="ended" {{ old('status', $event->status) === 'ended' ? 'selected' : '' }}>Selesai</option>
                </select>
        ...
        <div class="mb-6">
            ...
            <textarea name="rewards_json" rows="5" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder='[{"rank":1,"reward_type":"cash","reward_value":500000,"description":"Juara 1"}]'>{{ old('rewards_json', $event->rewards ? json_encode($event->rewards, JSON_PRETTY_PRINT) : '') }}</textarea>
        ...
        <div class="flex justify-end">
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white text-sm rounded-xl hover:bg-primary-700">Perbarui Event</button>
        </div>
    </form>
</div>
@endsection
